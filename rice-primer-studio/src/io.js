...
async function eachLine(file,callback,progress=()=>{},hash=null){let carry='',offset=0;const decoder=new TextDecoder('utf-8',{fatal:true});const chunkSize=2**20;for(let start=0;start<file.size;start+=chunkSize){const bytes=new Uint8Array(await file.slice(start,start+chunkSize).arrayBuffer());if(hash)hash.update(bytes);const s=carry+decoder.decode(bytes,{stream:true});let from=0;while(true){const n=s.indexOf('\n',from);if(n<0)break;const raw=s.slice(from,n+1);callback(raw.replace(/\r?\n$/,''),offset,raw.length);offset+=raw.length;from=n+1;}carry=s.slice(from);assert(carry.length<32*1024*1024,'单行 FASTA 超过 32 MB；请先用 seqkit seq -w 60 等工具规则换行。');progress(Math.min(1,(start+bytes.length)/file.size));}carry+=decoder.decode();if(carry)callback(carry,offset,carry.length);}
...
async function scanExact(file,queries,progress=()=>{}){const RPS=root.RPS||(typeof require!=='undefined'?require('./engine.js'):null);const normalized=queries.map(q=>({...q,sequence:q.sequence.toUpperCase()}));normalized.forEach(q=>assert(/^[ACGT]+$/.test(q.sequence),'比对引物只能包含 A/C/G/T。'));
 ...
 let chrom=null,buffer='',total=0,keep='';const seen=new Set();
 ...
 await eachLine(file,(line)=>{if(line.startsWith('>')){flush();chrom=line.slice(1).trim().split(/\s+/)[0];assert(!seen.has(chrom),'FASTA 中有重复 contig。');seen.add(chrom);total=0;keep='';}else{assert(chrom&&/^[ACGTRYSWKMBDHVNacgtryswkmbdhvn]+$/.test(line),'FASTA 格式错误。');buffer+=line;if(buffer.length>=262144)flush();}},progress);flush();return {method:'FULL_LENGTH_EXACT_MATCH_BOTH_STRANDS',referenceScope:'UPLOADED_FASTA_ONLY',mismatchesAllowed:0,gapsAllowed:false,tailIncluded:false,contigsScanned:seen.size,results,warning:'只统计完整结合区的零错配命中；未检查近似匹配、错配延伸或真实 PCR。ALT 引物在 MSU 上无命中可能是预期现象。'};}
...
async function scanApprox(file,queries,maxMismatches=2,progress=()=>{},options={}){
 ...
 await eachLine(file,line=>{
  if(line.startsWith('>')){flush(true);chrom=line.slice(1).trim().split(/\s+/)[0];assert(chrom&&!seen.has(chrom),'FASTA contig 名为空或重复。');seen.add(chrom);total=0;keep='';}
