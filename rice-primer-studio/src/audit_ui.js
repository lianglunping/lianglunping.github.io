...
function parameterAuditHTML(r,c){
 const a=RPSAudit.parameterEvidence(r,c),p=r.parameters||{};
 const tmNote=a.tmEvidence==='RPS_NN_RECOMPUTED'?'已用本次保存的盐/浓度参数重算离线 NN Tm':'仅核对原生返回 Tm 的范围与差值；未完成 Primer3 数值验证';
 return `<details class="parameter-audit" style="margin:12px 0"><summary>设计条件复核 · ${esc(a.status)} · 使用本次结果参数快照</summary><div class="hint">${esc(tmNote)}。结构指标用含尾序的完整寡核苷酸重算；下表不是热力学 ΔG 或实验成功判定。</div>${a.errors.length?`<div class="notice error">${a.errors.map(esc).join('；')}</div>`:''}<div class="table-wrap"><table><thead><tr><th>角色</th><th>结合区 nt</th><th>GC %</th><th>Tm / °C</th><th>重算 Tm</th><th>最长同聚 nt</th><th>3′自互补 nt</th><th>发卡茎 nt</th></tr></thead><tbody>${a.rows.map(o=>`<tr><td>${esc(o.role)}</td><td>${o.coreLength}</td><td>${fmt(o.gc,2)}</td><td>${fmt(o.reportedTm,3)}</td><td>${fmt(o.expectedTm,3)}</td><td>${o.homopolymer}</td><td>${o.selfEnd}</td><td>${o.hairpin}</td></tr>`).join('')}</tbody></table></div><div class="hint">本次条件：长度 ${esc(p.minLen)}–${esc(p.maxLen)} nt；GC ${esc(p.gcMin)}–${esc(p.gcMax)}%；Tm ${esc(p.tmMin)}–${esc(p.tmMax)}°C；本组最大 Tm 差 ${fmt(a.pairTmSpan,3)} / 允许 ${esc(p.maxTmDiff)}°C；重算最大 3′交叉互补 ${esc(a.crossEnd)} / 允许 ${esc(p.maxPairEnd)} nt。</div></details>`;
}
cardHTML=function(r,c,ri,ci){return oldCardHTML(r,c,ri,ci) + parameterAuditHTML(r,c) + kaspAuditHTML(r,c)};
function readiness(){
